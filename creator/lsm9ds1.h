...
class LSM9DS1 {
 public:
  ...
  // accel/gyroscope.
  // 				If IMU_MODE_SPI, this is the chip select pin of the
  // gyro
  // (CS_AG)
  ...
  void configAccelThs(uint8_t threshold, lsm9ds1_axis axis,
  ...
  //	  true: Wait for duration samples before exiting interrupt
  //	  false: Wait function off
  void configGyroThs(int16_t threshold, lsm9ds1_axis axis, uint8_t duration,
                     bool wait);
