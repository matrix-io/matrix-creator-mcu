...
// This is a function that uses the FIFO to accumulate sample of accelerometer
// and gyro data, average
// them, scales them to  gs and deg/s, respectively, and then passes the biases
// to the main sketch
// for subtraction from all subsequent data. There are no gyro and accelerometer
// bias registers to store
// the data as there are in the ADXL345, a precursor to the LSM9DS0, or the
// MPU-9150, so we have to
// subtract the biases ourselves. This results in a more accurate measurement in
// general and can
// remove errors due to imprecise or varying initial placement. Calibration of
// sensor data in this manner
// is good practice.
...
// TODO (yoel.castillo): setMagOffsetZ is not working , no idea why yet
...
void LSM9DS1::configInt(interrupt_select interrupt, uint8_t generator,
  ...
  // those two values.
  // [generator] should be an OR'd list of values from the interrupt_generators
  // enum
...
void LSM9DS1::configAccelThs(uint8_t threshold, lsm9ds1_axis axis,
...
void LSM9DS1::configGyroThs(int16_t threshold, lsm9ds1_axis axis,
                            uint8_t duration, bool wait) {
  uint8_t buffer[2];
...
uint8_t LSM9DS1::getFIFOSamples() { return (xgReadByte(FIFO_SRC) & 0x3F); }
